...
## [2.0.2] - 2026-06-25 · 桌面端功能对齐移动端

> 将移动端已有功能补齐到 Windows 桌面端，使两端体验一致。

### ✨ 新增（桌面端）

- **日历视图**：在「列表 / 看板」旁新增「日历」。月历格高亮当天与有计划的日期（按类型显示彩色圆点），点击日期在右侧查看当天计划，「+ 新建计划」一键在左侧录入栏预填该日提醒时间。完全跟随主题（浅色/深色）与界面风格（玻璃/杂志/扁平）自适应。
- **Esc 关闭浮层**：对应移动端返回键，按 Esc 依次关闭 图片预览 / 版本历史 / 设置 浮层（桌面端本就关闭到托盘常驻，等价后台保活）。

### 🔧 其它

- 升级 `versionCode` 至 `4`，`versionName` 与桌面版本至 `2.0.2`。
- 移动端通过 `is-mobile` 隐藏桌面日历容器并将 `calendar` 视图回退为列表，避免与移动端独立日历 Tab 冲突。
